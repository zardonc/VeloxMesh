...
def test_worker_calls_onnxruntime_session_and_returns_quantiles(tmp_path):
    ...
    try:
        ...
        response = stub.BatchPredict(predictor_pb2.BatchPredictRequest(tasks=[predictor_pb2.TaskFeature(task_id="ok")]))

        assert health.ready is True
        assert response.predictions[0].quantiles[50] == 16
        assert response.predictions[0].quantiles[70] == 20
        assert response.predictions[0].quantiles[90] == 24
        assert response.predictions[0].signals["quantile_spread"] == 8
    finally:
        server.stop(0)
...
def test_worker_reports_malformed_task_without_blocking_siblings(tmp_path):
    ...
    try:
        ...
        assert response.predictions[1].quantiles[70] == 20
    finally:
        server.stop(0)

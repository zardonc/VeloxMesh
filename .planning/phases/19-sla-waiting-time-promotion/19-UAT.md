...
number: 1
name: Production-like full verification baseline
expected: |
  In a clean local checkout that simulates the next production gate, `go test -timeout 60s ./...` and `go build ./...` both complete successfully. No Phase 19 test depends on prompt text, secrets, provider payloads, embeddings, or semantic-cache payloads.

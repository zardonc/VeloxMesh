...
## Follow-up: 26-03 Verification

source: User requested `$gsd-verify-work 26-03` with real resources, no mocks, and concurrency testing.
observation: Focused and full verification initially exposed a flaky real TCP scorer success-path test: `TestGRPCScorerCallsRealSchedulerOverTCP` used a 15ms total deadline and could fall back with `timeout` under parallel package load.
fix: Successful real TCP scorer tests now use a named 100ms test deadline, while the dedicated timeout fallback test keeps the short 15ms deadline.
verification: `go test -timeout 60s ./internal/scheduler -run TestTaskIntakeConcurrent -count=1 -v`; `go test -timeout 60s ./internal/scheduler -run SubmittedRequestContext -count=1 -v`; `go test -timeout 60s ./internal/scheduler -run TestExtractSafeFeaturesChineseSentenceLengthBucket -count=1 -v`; `go test -timeout 60s ./internal/http/handlers -run TestAdminSchedulerRollout -count=1`; `go test -timeout 60s ./cmd/scheduler -run TestSchedulerServiceUsesPythonONNXWorkerSmoke -count=1 -v`; `go test -timeout 60s ./cmd/scheduler -run TestSchedulerServicePredictorBreakerEnvUsesRealTCP -count=1 -v`; `PATH=/c/Soft/1A-Coding/w64devkit/bin:$PATH go test -race -timeout 60s ./internal/scheduler ./internal/scheduler/predictor -count=1`; `go test -timeout 60s ./internal/scheduler ./internal/http/handlers ./cmd/scheduler -count=1`; `go test -timeout 60s ./... -count=1`; `go build ./...`; `git diff --check`; `go test -timeout 60s ./tests/integration -run TestPlan4PostgresSansPrimaryRealProviderSmoke -count=1 -v`.
